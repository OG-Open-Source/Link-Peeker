...
function applyAppearance(shadowRoot) {
	...
	if (themeConfig) {
		...
		if (box) {
			box.style.backdropFilter = themeConfig.blurEnabled ? `blur(${themeConfig.blurAmount}px)` : 'none';
			box.style.webkitBackdropFilter = themeConfig.blurEnabled ? `blur(${themeConfig.blurAmount}px)` : 'none';
		}
		...
		shadowRoot.host.style.setProperty('--button-hover-bg', themeConfig.buttonHoverBg);
		shadowRoot.host.style.setProperty('--primary-text-color', themeConfig.primaryTextColor);
		shadowRoot.host.style.setProperty('--secondary-text-color', themeConfig.secondaryTextColor);
...
document.addEventListener(
	...
	async (event) => {
		...
		previewWindow.querySelector('#link-peeker-open').addEventListener('click', () => {
			window.open(link.href, '_blank');
			closeWindow();
		});
		previewWindow.querySelector('#link-peeker-close').addEventListener('click', closeWindow);
		...
		if (iframe) {
			...
			const showError = (messageKey, substitutions) => {
				...
				errorMessage.textContent = chrome.i18n.getMessage(messageKey, substitutions || []) || messageKey;
			};
			...
			chrome.runtime.sendMessage({ type: 'resolveUrl', url: link.href }, (response) => {
				if (chrome.runtime.lastError) {
					console.error('Link Peeker: Error resolving URL.', chrome.runtime.lastError);
				...
				loadingUrl.textContent = chrome.i18n.getMessage('statusLoading', [finalUrl]);

...
chrome.runtime.onInstalled.addListener(async () => {
	...
	try {
		...
		console.log(`Link Peeker: Attempted to remove lingering session rule with ID ${UPGRADE_RULE_ID}.`);
	} catch (e) {
		console.warn(`Link Peeker: Could not remove session rule ${UPGRADE_RULE_ID}. It might not have existed.`, e);
	}
	...
	try {
		...
		console.log(`Link Peeker: Successfully added session rule ${UPGRADE_RULE_ID} to upgrade HTTP requests.`);
	} catch (e) {
		console.error(`Link Peeker: CRITICAL - Failed to add session rule ${UPGRADE_RULE_ID}.`, e);
	}
		...
		fetch(originalUrl, { method: 'GET', redirect: 'follow' })
			.then((response) => {
				...
				try {
					...
					const initiatorHost = initiatorUrl ? new URL(initiatorUrl).hostname : '';
					...
					if (finalUrlHost.includes('youtube.com') && finalUrl.includes('/watch') && !initiatorHost.includes('youtube.com')) {

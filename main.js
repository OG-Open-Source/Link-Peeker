...
function populateThemeDropdown(themes, selectedTheme) {
	...
	const customThemeKeys = Object.keys(themes).filter((key) => key !== 'light' && key !== 'dark');
...
document.addEventListener('DOMContentLoaded', () => {
	const elements = {
		...
		themeButtonHoverBgInput: document.getElementById('theme-buttonHoverBg-input'),
		themePrimaryTextColorInput: document.getElementById('theme-primaryTextColor-input'),
		themeSecondaryTextColorInput: document.getElementById('theme-secondaryTextColor-input'),
	...
	loadSettings(async (settings) => {
		...
		Object.keys(defaultSettings.sandbox).forEach((key) => {
			createPolicyOption('checkbox', key, `sb_${key.replace(/-/g, '_')}`, elements.sandboxOptions);
		});
		Object.keys(defaultSettings.allow).forEach((key) => {
			createPolicyOption('checkbox', key, `fp_${key.replace(/-/g, '_')}`, elements.allowOptions);
		});
		REFERRER_POLICIES.forEach((key) => {
			createPolicyOption('radio', key, `rp_${key.replace(/-/g, '_')}`, elements.referrerPolicyOptions);
		});

		// Set saved values
		Object.keys(settings.sandbox).forEach((key) => {
			const checkbox = elements.sandboxOptions.querySelector(`[data-key="${key}"]`);
			if (checkbox) checkbox.checked = settings.sandbox[key];
		});

		Object.keys(settings.allow).forEach((key) => {
			const checkbox = elements.allowOptions.querySelector(`[data-key="${key}"]`);
			if (checkbox) checkbox.checked = settings.allow[key];
		});

		const radio = elements.referrerPolicyOptions.querySelector(`[value="${settings.referrerPolicy}"]`);
	...
	// --- EVENT LISTENERS ---
	elements.triggerKey.addEventListener('change', (e) => saveSettings({ triggerKey: e.target.value }));
	...
	const showThemeEditor = (themeData = null) => {
		if (themeData) {
			...
			elements.themeSecondaryTextColorInput.value = themeData.secondaryTextColor;
	...
	elements.deleteThemeBtn.addEventListener('click', async () => {
		...
		if (selectedTheme && selectedTheme !== 'light' && selectedTheme !== 'dark') {
			if (confirm(`Are you sure you want to delete the theme "${allThemes[selectedTheme].name}"?`)) {
	...
	elements.importThemeBtn.addEventListener('click', () => elements.importThemeInput.click());
	...
	function renderCustomRules(rules) {
        ...
        <td class="action-buttons">
          <button class="button is-small edit-rule-btn" data-index="${index}" title="${chrome.i18n.getMessage('editRuleBtn')}">
            <i data-lucide="square-pen"></i>
          </button>
          <button class="button is-danger is-small delete-rule-btn" data-index="${index}" title="${chrome.i18n.getMessage('deleteRuleBtn')}">
	...
	elements.addRuleForm.addEventListener('submit', async (e) => {
		...
		if (elements.ruleRemoveCsp.checked) headersToRemove.push('Content-Security-Policy');
		if (elements.ruleRemoveXfo.checked) headersToRemove.push('X-Frame-Options');

		if (headersToRemove.length === 0) {
			alert(chrome.i18n.getMessage('error_select_header') || 'Please select at least one header to remove.');
			return;
		}
		...
		if (ruleIndex > -1) {
			...
			if (existingRules.some((rule) => rule.domain === domain)) {
				alert(chrome.i18n.getMessage('error_duplicate_rule') || 'A rule for this domain already exists.');
				return;
			}
	...
	elements.customRulesTableBody.addEventListener('click', async (e) => {
		...
		if (editButton) {
			...
			elements.ruleRemoveCsp.checked = ruleToEdit.headersToRemove.includes('Content-Security-Policy');
			elements.ruleRemoveXfo.checked = ruleToEdit.headersToRemove.includes('X-Frame-Options');
		...
		if (deleteButton) {
			...
			if (confirm(chrome.i18n.getMessage('confirm_delete_rule', [ruleToDelete.domain]) || `Are you sure you want to delete the rule for ${ruleToDelete.domain}?`)) {
				const updatedRules = currentSettings.customRules.filter((_, index) => index !== indexToDelete);
				await updateCustomRules(updatedRules);
			}
		}
	});
	...
	sidebarLinks.forEach((link) => {
		link.addEventListener('click', (e) => {
			...
			const activeSection = document.querySelector('.content-section.is-active');
